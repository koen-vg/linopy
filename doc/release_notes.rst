...
* ``read_netcdf`` no longer silently mis-maps frozen (CSR) constraint coefficients when the variables were not added in alphabetical order. Frozen constraint columns are dense positions in the active-variable ordering (variable insertion order); ``to_netcdf`` now records that order in the ``_variable_order`` attribute and ``read_netcdf`` rebuilds variables accordingly, instead of sorting them by name. Legacy files without the attribute keep the previous sorted-order behaviour.
